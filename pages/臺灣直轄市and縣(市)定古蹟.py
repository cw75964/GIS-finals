...
nation_old=old[(old['assetsClassifyName']=='直轄市定古蹟')|(old['assetsClassifyName']=='縣(市)定古蹟')]
ci_na=old_na[(old_na['assetsClassifyName']=='直轄市定古蹟')|(old_na['assetsClassifyName']=='縣(市)定古蹟')]
ci_name=ci_na['name'].value_counts()
ci_city=ci_na['city'].value_counts()

m1 = leafmap.Map(center=[23.7652,120.4980],zoom=8,
            locate_control=True, latlon_control=True, draw_export=True, minimap_control=True)
m1.add_basemap("OpenTopoMap")
m1.add_points_from_xy(nation_old,x='longitude',y='latitude',spin=True,add_legend=True,layer_name='直轄市and縣(市)定古蹟')
m1.to_streamlit(height=700)
st.dataframe(nation_old)
st.bar_chart(ci_name)
st.header("Heatmap")
nation_old['num']=10
