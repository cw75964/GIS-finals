...
dwell_old=old_na[old_na['name']=='產業']
dwell_build=build_na[build_na['name']=='產業']
...
st.header("產業(古蹟)Marker Cluster")

m1 = leafmap.Map(center=[23.7652,120.4980],zoom=8,
            locate_control=True, latlon_control=True, draw_export=True, minimap_control=True)
m1.add_basemap("OpenTopoMap")
m1.add_points_from_xy(old_maker,x='longitude',y='latitude',spin=True,add_legend=True,layer_name='產業(古蹟)')
m1.to_streamlit(height=700)
st.dataframe(old_maker)
st.header("產業(古蹟)Heatmap")
dwell_old['num']=10
...
st.header("產業(歷史建築)Marker Cluster")

m3 = leafmap.Map(center=[23.7652,120.4980],zoom=8,
            locate_control=True, latlon_control=True, draw_export=True, minimap_control=True)
m3.add_basemap("OpenTopoMap")
m3.add_points_from_xy(build_maker,x='longitude',y='latitude',spin=True,add_legend=True,layer_name='產業(歷史建築)')
m3.to_streamlit(height=700)
st.dataframe(build_maker)
st.header("產業(歷史建築)Heatmap")
dwell_build['num']=10
